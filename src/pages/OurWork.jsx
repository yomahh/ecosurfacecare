      ...
      <section className="pt-10 pb-6 lg:pt-12 lg:pb-10">
        <div className="container-site">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {items.map(item => <article key={item.id} className="overflow-hidden rounded-3xl border border-slate-200"><img src={item.image} alt={item.title} className="aspect-square w-full object-cover" /><div className="p-6"><p className="text-sm font-bold text-[#176B1C]">{item.category}</p><h2 className="mt-2 text-xl font-bold">{item.title}</h2><p className="mt-2 text-slate-500">{item.location}</p></div></article>)}
          </div>
        </div>
      </section>
    </>
