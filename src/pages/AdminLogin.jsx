  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#f5faf7] p-5">
      <section className="w-full max-w-md rounded-3xl bg-white p-8 shadow-xl">
        <div className="grid h-12 w-12 place-items-center rounded-2xl bg-emerald-50 text-[#0b6f63]"><LockKeyhole /></div>
        ...
        <form className="mt-7 grid gap-4" onSubmit={(e)=>{e.preventDefault();navigate("/admin/dashboard");}}>
          ...
          <button className="rounded-full bg-[#0b6f63] px-6 py-3 font-bold text-white">Continue to dashboard</button>
        </form>
        <Link to="/" className="mt-6 block text-center font-semibold text-[#0b6f63]">Return to website</Link>
      </section>
    </main>
  );
}
