      ...
      <section className="section-space">
        <div className="container-site grid gap-10 lg:grid-cols-2">
          ...
          <div className="grid gap-5">
            {[[BadgeCheck,"Trusted methods","Authorised partner positioning for GroutGleam and BioSteam."],[Eye,"Attention to detail","Careful preparation, neat application and clear finishing checks."],[HeartHandshake,"Customer-first service","Professional communication from first enquiry through completion."]].map(([Icon,title,text]) => <div key={title} className="rounded-3xl border border-slate-200 p-6"><Icon className="text-[#0b6f63]"/><h3 className="mt-4 text-xl font-bold">{title}</h3><p className="mt-2 leading-7 text-slate-600">{text}</p></div>)}
          </div>
        </div>
      </section>
    </>
